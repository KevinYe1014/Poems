...
flags.DEFINE_string('checkpoints_dir', os.path.abspath('./checkpoints/lyrics'), 'checkpoint dir')
flags.DEFINE_string('file_path', os.path.abspath('./dataset/data/周杰伦歌词大全.txt'), 'data file')
flags.DEFINE_string('model_prefix', 'lyrics', 'model save prefix')
flags.DEFINE_integer('epochs', 500, 'train how many epochs. ')
...
def run_training():
    ...
    lyrics_vector, word_to_int, vocabularies = process_lyrics(FLAGS.file_path)
    batches_inputs, batches_outputs = generate_batch(FLAGS.batch_size, lyrics_vector, word_to_int)
    ...
    with tf.Session() as sess:
        ...
        if checkpoint:
            ...
            print('[INFO] restore from the checkpoint %s' % FLAGS.checkpoints_dir)
            start_epoch += int(checkpoint.split('-')[-1])
        print('[INFO] start training ... ')
        try:
            for epoch in range(start_epoch, FLAGS.epochs):
                n = 0
                n_chunk = len(lyrics_vector) // FLAGS.batch_size
...
def gen_lyrics():
    batch_size = 1
    lyrics_vector, word_int_map, vocabularies = process_lyrics(FLAGS.file_path)
